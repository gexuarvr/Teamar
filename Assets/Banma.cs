...
public class Banma : MonoBehaviour {
	...
	void Rotating (float horizontal, float vertical)
	{
		...
		Debug.Log (Vector3.Distance (gameObject.transform.position, postionV)  +"--"+postionV.x+"---"+postionV.y + "----"+gameObject.transform.position.x+"---"+gameObject.transform.position.z);
		Vector3 targetDirection;
		if (Vector3.Distance (gameObject.transform.position, postionV) > 2f) {
